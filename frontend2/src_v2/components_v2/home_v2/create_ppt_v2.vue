...
const generateFile = async () => {
  try {
    ...
    const response = await axios.post('http://localhost:8080/api/create_ppt', {
...
const viewRecentPDF = async () => {
  try {
    const response = await axios.get('http://localhost:8080/api/recent_file');
    if (response.data.filename) {
      const fileUrl = `http://localhost:8080/api/display_file?file=${response.data.filename}`;
